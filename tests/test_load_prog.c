...
static void put_be16(FILE* f, uint16_t v) { fputc((v >> 8) & 0xFF, f); fputc(v & 0xFF, f); }
...
static const char* make_prog(uint16_t start, uint16_t first, uint16_t last,
    ...
    if (fd < 0) return NULL;
    FILE* f = fdopen(fd, "wb");
    put_be16(f, start); put_be16(f, start); /* start, restart */
    put_be16(f, first); put_be16(f, last);
    put_be16(f, fB2);   put_be16(f, lB2);
    for (int i = 12; i < 512; i++) fputc(0, f);   /* pad header block */
    for (int i = 0; i < n; i++) put_be16(f, data[i]);
    fclose(f);
    return path;
}
...
static void test_one_bank(void) {
    ...
    CHECK(h.startAddress == 0100 && h.firstBank1 == 0200 && h.lastBank1 == 0203,
          ...
          h.startAddress, h.firstBank1, h.lastBank1);
