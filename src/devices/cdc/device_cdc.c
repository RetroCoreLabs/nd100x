/*
 * device_cdc.c - CDC/NCR cartridge disc controller for NORD TSS (IOX 500-507).
 *
 ...
 * See device_cdc.h for the full register map and bit model (with manual
 * citations) and <TSS>\docs\CDC-DISC-DEVICE.md for the wire
 * protocol. Modelled structurally on src/devices/drum/device_drum.c and
 ...
 * ADDRESSING NOTE in device_cdc.h. Kept as a tiny named function so the seam is
 * unmistakable.
 * ------------------------------------------------------------------------------ */
...
static bool Cdc_End(void *context, int param)
{
     ...
     * Device_SetInterruptStatus, exactly like device_floppy_dma.c ReadEnd()
     * (device_floppy_dma.c:742) and device_smd.c SMDReadEnd() (device_smd.c:1147).
     *
...
static uint16_t Cdc_Ident(Device *self, uint16_t level)
{
    ...
    if ((self->interruptBits & (1 << level)) != 0)
    {
         ...
         * Ident (device_floppy_dma.c:248 clears status1.interruptEnabled then
         * Device_SetInterruptStatus(false); device_smd.c:675 the same). Leaving
...
static int Cdc_Boot(Device *self, int unit)
{
    ...
    if (!d || !d->surface || d->surfaceWords < CDC_WORDS_PER_SECTOR)
    {
        LOG(LOG_CAT_CDC, LOG_ERROR,
            "Error: CDC boot - no disc surface attached (use --cdc=FILE)\n");
        return -1;
    }
...
Device *CreateCdcDevice(uint8_t thumbwheel)
{
    ...
    LOG(LOG_CAT_CDC, LOG_INFO,
        "CDC disc device created: %s ident %o level %d (%u sectors, %u words surface)\n",
        dev->memoryName, dev->identCode, dev->interruptLevel, d->surfaceSectors, d->surfaceWords);
    return dev;
}
