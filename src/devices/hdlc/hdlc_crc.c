/*
 * hdlc_crc.c - HDLC CRC-16, CCITT CRC and parity bit calculation.
 *
...
// CRC16 polynomial x^16 + x^15 + x^2 + 1 (0xA001) CCITT LSB nibble lookup table
static const uint16_t crc16_nibble[16] = {0x0000, 0xCC01, 0xD801, 0x1400, 0xF001, 0x3C00,
...
uint16_t HDLC_CRC_CalcCrc16(uint16_t crc, uint8_t byte)
{
    ...
    crc = (uint16_t)((crc >> 4) ^ crc16_nibble[(byte ^ crc) & 0xF]);
    crc = (uint16_t)((crc >> 4) ^ crc16_nibble[((byte >> 4) ^ crc) & 0xF]);
    return crc;
}
...
uint8_t HDLC_CRC_CalculateParityBit(uint8_t data, HDLCParityMode mode)
{
    ...
    if (mode == HDLC_PARITY_ODD)
    {
        // For odd parity, return 1 if count is even (to make total odd), 0 if count is odd
        return (uint8_t)((count % 2 == 0) ? 1 : 0);
    }
    else if (mode == HDLC_PARITY_EVEN)
    {
        // For even parity, return 0 if count is even (to keep total even), 1 if count is odd
        return (uint8_t)((count % 2 == 0) ? 0 : 1);
    }
