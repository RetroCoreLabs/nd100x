...
static void fdb_ensure_cache_dir(void)
{
    ...
    if (!home) return;
    char d[600];
    snprintf(d, sizeof(d), "%s/.cache", home);       FDB_MKDIR(d);
    snprintf(d, sizeof(d), "%s%s", home, FDB_CACHE_SUFFIX); FDB_MKDIR(d);
}
...
{
    if (!content || !label || !out || out_len == 0) return false;
    out[0] = '\0';
...
{
    char buf[64];
    if (!fdb_extract_field(content, "Filesystem image size", buf, sizeof(buf))) return -1;
...
static char *fdb_get_json(bool force_refresh)
{
    ...
    if (!force_refresh && path) {
        ...
        if (age >= 0 && age < FDB_CACHE_MAX_AGE) {
            ...
            if (cached) { free(path); return cached; }
        }
    }
...
int floppydb_load_json(const char *json_text)
{
    ...
    if (!s_entries) { cJSON_Delete(root); return -1; }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (status && cJSON_IsNumber(status) && status->valueint != 0) continue; /* Status 0 only */
        ...
        const char *name_s = (name && cJSON_IsString(name) && name->valuestring) ? name->valuestring : "";
...
int floppydb_count(void) { return s_count; }
...
{
    if (!directory_name) return 0;
    int matches = 0;
    for (int i = 0; i < s_count; i++) {
        if (strcasecmp(s_entries[i].directory_name, directory_name) == 0) {
            if (out && matches < max) out[matches] = &s_entries[i];
            matches++;
        }
    }
    return matches;
