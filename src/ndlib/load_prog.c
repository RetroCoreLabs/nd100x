/*
 * load_prog.c - SINTRAN :PROG loadable-image loader (one or two banks).
 *
...
static int load_bank(FILE *f, uint16_t first, uint16_t last, bool verbose, const char *label)
{
    ...
    if (count <= 0)
    {
        if (verbose)
        {
            LOG(LOG_CAT_LOADER, LOG_INFO, "  %s: no data (first=0o%o last=0o%o)", label, first,
                last);
        }
        return 0;
    }
    if (verbose)
    {
        LOG(LOG_CAT_LOADER, LOG_INFO, "  %s: loading %d words at 0o%o..0o%o", label, count, first,
            last);
    }
...
int prog_load(const char *filename, bool verbose)
{
    ...
    if (hdr.twoBank)
    {
        ...
        LOG(LOG_CAT_LOADER, LOG_INFO,
            "PROG load: 2-bank :PROG not yet supported (Bank 2 needs the "
