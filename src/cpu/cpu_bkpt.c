                      ...
                      const char *hit_condition, const char *log_message)
{
    ...
    entry->hitCondition = hit_condition ? strdup(hit_condition) : NULL;
    entry->logMessage = log_message ? strdup(log_message) : NULL;
    entry->hitCount = 0;
...
static int breakpoint_manager_check(uint16_t address, BreakpointEntry **matches[], int *match_count)
{
    int h = hash_address(address);
    BreakpointEntry *curr = g_breakpoint_mgr->buckets[h];

    BreakpointEntry *temp_list[10];
    int temp_count = 0;
    BreakpointEntry *user_list[10];
    int user_count = 0;
    ...
    if (temp_count > 0)
    {
        *matches = malloc(sizeof(BreakpointEntry *) * temp_count);
        if (!*matches)
        {
            *match_count = 0;
            return 0;
        }
        memcpy(*matches, temp_list, sizeof(BreakpointEntry *) * temp_count);
        *match_count = temp_count;
    }
    else if (user_count > 0)
    {
        *matches = malloc(sizeof(BreakpointEntry *) * user_count);
        if (!*matches)
        {
            *match_count = 0;
            return 0;
        }
        memcpy(*matches, user_list, sizeof(BreakpointEntry *) * user_count);
        *match_count = user_count;
    }
    else
    {
        *matches = NULL;
...
int bkpt_check_hit(void)
{
    ...
    int hit_count;
    BreakpointType bt_type = BT_NONE;
    uint16_t pc = gPC;
    ...
    if (breakpoint_manager_check(pc, &hits, &hit_count))
    {
        for (int i = 0; i < hit_count; i++)
        {
            ...
            if (bp->hitCondition)
            {
                int hit_cond_val = (int)strtol(bp->hitCondition, NULL, 10);
                hit_ok = (bp->hitCount == hit_cond_val);
            }
...
static CpuStopReason stop_reason_from_breakpoint(BreakpointType t)
{
    ...
    case BP_TYPE_FUNCTION:
        return STOP_REASON_FUNCTION_BREAKPOINT;
    case BP_TYPE_DATA:
        return STOP_REASON_DATA_BREAKPOINT;
    case BP_TYPE_INSTRUCTION:
        return STOP_REASON_INSTRUCTION_BREAKPOINT;
    default:
        return STOP_REASON_BREAKPOINT;
    }
}
...
bool bkpt_manager_get_last_hit(uint16_t *address)
{
    if (g_breakpoint_mgr && g_breakpoint_mgr->last_hit_valid)
    {
        if (address)
        {
            *address = g_breakpoint_mgr->last_hit_address;
        }
...
static void watchpoint_bitmap_rebuild(void)
{
    ...
    for (int i = 0; i < g_watchpoint_count; i++)
    {
        ...
        {
            g_watchpoint_bitmap[s_watchpoints[i].address >> 3] |=
                (1 << (s_watchpoints[i].address & 7));
        }
    }
}
...
int bkpt_watchpoint_check_slow(uint16_t address, bool is_write, bool use_apt)
{
    ...
    for (int i = 0; i < g_watchpoint_count; i++)
    {
        ...
        if (w->pil >= 0 && w->pil != cur_pil)
        {
            continue;
        }
        if (w->space == WATCH_SPACE_ISPACE && use_apt)
        {
            continue;
        }
        if (w->space == WATCH_SPACE_DSPACE && !use_apt)
        {
            continue;
        }
        WatchpointType t = w->type;
        int matched = (t == WATCH_READWRITE) || (is_write && t == WATCH_WRITE) ||
                      (!is_write && t == WATCH_READ);
        if (matched)
        {
...
int bkpt_phys_watchpoint_add(uint32_t address, WatchpointType type, int8_t pil)
{
    ...
    if (g_phys_watchpoint_count >= MAX_WATCHPOINTS)
    {
        return -1;
    }
...
int bkpt_phys_watchpoint_check(uint32_t address, bool is_write)
{
    ...
    for (int i = 0; i < g_phys_watchpoint_count; i++)
    {
        ...
        if (phys_watchpoints[i].address != address)
        {
            continue;
        }
        if (phys_watchpoints[i].pil >= 0 && phys_watchpoints[i].pil != cur_pil)
        {
            continue;
        }
        WatchpointType t = phys_watchpoints[i].type;
        if (t == WATCH_READWRITE)
        {
            return 1;
        }
        if (is_write && (t == WATCH_WRITE))
        {
            return 1;
        }
        if (!is_write && (t == WATCH_READ))
        {
            return 1;
        }
    }
...
int bkpt_phys_watchpoint_get(int index, uint32_t *out_addr, int *out_type)
{
    ...
    if (!phys_watchpoints[index].active)
    {
        return -1;
    }
